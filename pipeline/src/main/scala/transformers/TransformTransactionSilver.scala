...
class TransformTransactionSilver(hudiService: HudiService, mongoDbService: MongoDbService) extends Transform {
  ...
  override def stgSilver(df: DataFrame): DataFrame = {
    ...
    transaction = transaction.select(col("after_value.after.*"),
  ...
  def resolve(transactionDf: DataFrame): DataFrame = {
    ...
    var customerLookup = mongoDbService.readMongo(customSchema, configVars.ATLAS_DATABASE, configVars.ATLAS_COLLECTION, configVars.atlasConnectionString());
    customerLookup = customerLookup.withColumn("timestamp", current_timestamp());
    var combinedDf = transactionDf.as("t").join(customerLookup.as("c"),
      expr(
        s"""|t.phone_number = c.phone_number
            |AND t.timestamp <= c.timestamp + interval 3 minutes
            |""".stripMargin),
      "inner"
    )
    var finalDf = combinedDf.select(col("c.customer_id"), col("t.*")).groupBy("c.customer_id").agg(collect_list(
        struct("transaction_id", "product_id", "product_name", "price", "quantity", "total_amount")
      )).alias("events")
    return finalDf;
  }
